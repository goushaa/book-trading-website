...
function AdminForm() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/drivers').then((res) => {
      setDrivers(res.data);
    ...
    axios.get('http://localhost:5000/pendingOrders').then((res) => {
  ...
  function addCoupon(e) {
    ...
    axios.post('http://localhost:5000/addCoupon', { code, discount, maximum_use, is_relative }).then(
      (res) => {
            ...
            <Row>
              <Form>
                <Form.Group className="mb-3 " controlId="couponform">
                  <Form.Control className='form-control w-75' type="string" maxLength={6} placeholder="Enter Coupon" onChange={changeCoupon} />
                </Form.Group>
                <Form.Group className="mb-3 " controlId="couponform">
                  <Form.Control className='form-control w-75' type="string" maxLength={6} placeholder="Enter Discount" onChange={changeDiscount} />
                </Form.Group>
                <Form.Group className="mb-3 " controlId="couponform">
                  <Form.Control className='form-control w-75' type="string" maxLength={6} placeholder="Enter Maximum Use" onChange={changeMaximumUse} />
                </Form.Group>
                <Form.Select className="type mt-3 mb-3" aria-label="Default select example" onChange={changeIsRelative}>
        ...
        >
          <Tab eventKey="Customers" title="Customers">
...

            {
              customers.map(customer => (
                <ul>
                  <li>{customer.email} <Link to={`/users/${customer.id}`}><Button variant="dark" > View </Button></Link></li>
                </ul>
              ))
            }
          ...
          <Tab eventKey="Stores" title="Stores">
...

            {
              stores.map(store => (
                <ul>
                  <li>{store.email} <Link to={`/stores/${store.id}`}><Button variant="dark" > View </Button></Link></li>
                </ul>
              ))
            }
          ...
          <Tab eventKey="Drivers" title="Drivers">
...

            {
              drivers.map(driver => (
                <ul>
                  <li>{driver.email} <Link to={`/drivers/${driver.ssn}`}><Button variant="dark" > View </Button></Link></li>
                </ul>
              ))
            }
          ...
          <Tab eventKey="Coupons" title="Coupons">
...

            {
              coupons.map(coupon => (
                <ul>
                  <li>{coupon.code} <Link to={`/coupons/${coupon.code}`}><Button variant="dark" > View </Button></Link> <Button variant="dark" onClick={function (e) {
                    axios.delete(`http://localhost:5000/coupons/${coupon.code}`).then(res => {
                      console.log(res);
                    }).catch(err => console.log(err));
                  }}> Delete</Button></li>
                </ul>
...

            {
              pendingOrders.map(pendingOrder => (
                <div>
                  ...
                  <Button variant="dark" onClick={function (e) {
                    setOrderID(pendingOrder.id)
                    axios.get(`http://localhost:5000/driveruseridgivenssn`, driver_ssn).then(res => {
                      setDriverID(res.data)
                    }).catch(err => console.log(err));
                    axios.post(`http://localhost:5000/AdminGiveOrderToDriver`, { driver_ssn, order_id, driver_user_id }).then(res => {
                      console.log(res);
                    }).catch(err => console.log(err));
                  }}> Assign</Button>
                </div>
