...
const [city_id, setCityID] = useState([]);
  const [cities, setCities] = useState([]);
    ...
    function savechanges(e)
    {
      ...
      axios.post('http://localhost:5000/updateUser', { first, last, address, city_id, user_name,id}).then(
            (res) => {
  ...
  return (
   <Fragment>
    ...
    <Navbar bg="dark" variant="dark">
        <Container>
          <Navbar.Brand ><h3>Account</h3></Navbar.Brand>
          <Navbar.Collapse className="justify-content-end">
          <Navbar.Text>
            Signed in as <Container className="username">{viewUser.user_name}</Container>
          </Navbar.Text>
        </Navbar.Collapse>
          </Container>
      </Navbar>
...
<Form.Control className=" box mt-1 mb-3" type="text" defaultValue={viewUser.first_name} onChange={changeFirst}/>
<div className="text">Last Name</div>
<Form.Control className=" box mt-1 mb-3" type="text" defaultValue={viewUser.last_name} onChange={changeLast} />
<div className="text">Address</div>
<Form.Control className=" box mt-1" type="text" defaultValue={viewUser.address} onChange={changeAddress} />
<div className="text">City</div>
<Form.Select className="box mt-1" aria-label="Default select example" onChange={changeCity}>
                                {
                                    cities.map(city => (
...
<Row>
  <Col>
<Link to={"../"}><Button className="logout mt-3">Log Out</Button></Link>
</Col>
<Col>
<Button className="save mt-3" onClick={savechanges}>Save Changes</Button>
</Col>
</Row>
</Container>
