    ...
    if (type == 4) {
      ...
      if (type == "2") {
        const order = await pool.query(
          `INSERT INTO "order" (user_id,status) VALUES ($1,$2) RETURNING *`,
          [sign.rows[0].id, 0]
        );
...
app.post("/makeOrder", async (req, res) => {
  try {
    ...
    const checkEmpty = await pool.query(
      "SELECT COUNT(order_id) FROM order_item where order_id=$1",
      [order_id]
    );
    ...
    const makeOrder = await pool.query(
      'UPDATE "order" SET status=1,order_date = $1,coupon_id =' +
        coupon_id +
...
app.get("/drivers", async (req, res) => {
  try {
    const viewDrivers = await pool.query(
      `SELECT * FROM driver d, "user" u WHERE u.type = 4;`
    );
